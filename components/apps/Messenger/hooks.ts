...
export const useNip05 = (): NIP05Result => {
  ...
  const updateNip05 = useCallback(async (): Promise<boolean> => {
    const response = await fetch("/contacts.json"); // Fetch from local file

    if (response.ok) {
      const { names = {} } = ((await response.json()) as NIP05Result) || {};
      setNip05({ names });
    }
...
// Hook to fetch and handle Nostr events
export const useNostrEvents = ({
  enabled = true,
  filter,
  onEvent,
  ...
  // Function to subscribe to a relay with a filter
  const subscribe = useCallback(
    (relay: Relay, subFilter: Filter[]): Sub => {
      const sub = relay.sub(subFilter);
  ...
  useEffect(() => {
    ...
    // Unsubscribe from relays on cleanup
    return () => relaySubs.forEach(({ sub }) => sub.unsub());
  }, [connectedRelays, enabled, filterString, subscribe]);
...
// Hook to fetch NIP-05 data
export const useNip05 = (): NIP05Result => {
  const [nip05, setNip05] = useState<NIP05Result>();

  // Function to update NIP-05 data
  const updateNip05 = useCallback(async (): Promise<boolean> => {
    const response = await fetch("/contacts.json"); // Fetch from local file
  ...
  // Function to fetch NIP-05 JSON data
  const fetchNip05Json = useCallback(async (): Promise<void> => {
    if (!(await updateNip05())) {
      setNip05({ relays: {} } as NIP05Result);
    }
...
// Hook to manage unread status and play notification sound
export const useUnreadStatus = (id: string, unreadCount: number): void => {
  const [currentUnreadCount, setCurrentUnreadCount] = useState(unreadCount);
  const { title } = useProcesses();
  const [pid] = id.split(PROCESS_DELIMITER);
...
// Hook to fetch the NIP-05 domain
export const useNip05Domain = (nip05?: string, publicKey?: string): string => {
  const [nip05Domain, setNip05Domain] = useState("");
...
// Hook to fetch and manage Nostr profile data
export const useNostrProfile = (
  publicKey: string,
  isVisible = true
): NostrProfile => {
